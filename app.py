...
if selected == "Report & Views":
    st.write("# :red[Reports & Views 1.8]")
    ...
    if questions == '1. What are the names of all the videos and their corresponding channels?':
        ...
        st.write(df)
        st.write("### :green[Top 10 most liked Channels & Videos :]")
        fig = px.bar(df,
                     x=columns[2],
                     y=columns[1],
                     orientation='h',
        ...
        data = cursor.execute("""SELECT channel_name AS Channel_Name
                            FROM video_details
        ...
        df = pd.DataFrame(col1, columns=columns)
        st.write(df)
        ...
        df = pd.DataFrame(list(zip(col1, col2)), columns=columns)
        st.write(df)
            
        st.write("### :green[Average video duration for channels :]")
        
    elif questions == '10. Which videos have the highest number of comments, and what are their corresponding channel names?':
        data = cursor.execute("""SELECT TOP 10 channel_name AS Channel_Name,video_id AS Video_ID,comments AS Comments
                            FROM video_details
